...
{
    return std::make_unique<SimpleTimer>(std::make_unique<SingleRunnerStrategy>(Timer::FunctionInfo{function, std::chrono::duration_cast<std::chrono::milliseconds>(interval)}));
}
...
{
    return std::make_unique<SimpleTimer>(std::make_unique<RecurringRunnerStrategy>(Timer::FunctionInfo{function, std::chrono::duration_cast<std::chrono::milliseconds>(interval)}));
}
